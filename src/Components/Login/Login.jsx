...
function Login() {
  ...
  const auth = getAuth(app); // Init auth early

  // 1. Start loading as TRUE to prevent "flash" of login form for logged-in users
  ...
  // 2. Logic to handle redirection (Moved up so useEffect can use it)
  ...
  // 3. NEW: Check if user is ALREADY logged in on mount
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (user) => {
      if (user) {
        // User is found! Redirect them immediately
        console.log("User already logged in, redirecting...");
        afterLogin(user);
      } else {
        // No user found, stop loading and show the form
        setLoading(false);
      }
    });
  ...
  return (
    <>
      <div className="cream min-h-screen global">
        ...
        {/* Only show the content if we are NOT in the initial loading check */}
        {/* This double check ensures the form doesn't flash if user is already logged in */}
